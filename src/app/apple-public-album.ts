...
export const getPublicAlbumId = (sharedAlbumUrl: string) => {
  ...
  const webStreamOrRedirectSchema = z.union([webStreamSchema, webStreamRedirectSchema]);
  
  type PublicAlbumWebStream = z.infer<typeof webStreamSchema>;
  
  export const fetchPublicAlbumWebStream = async (partition: string, albumId: string): Promise<{webStream: PublicAlbumWebStream, partition: string}> => {
    ...
    if ("X-Apple-MMe-Host" in result) {
      return fetchPublicAlbumWebStream(getHostPartition(result["X-Apple-MMe-Host"]), albumId);
    }
